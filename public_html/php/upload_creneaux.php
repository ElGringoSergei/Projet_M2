...
if(!isset($_POST['csrf'])) {
  ...
  if (file_exists($target_file)) {
    header("Location: ../reserver.php?error=Désolé, ce fichier existe déjà.");
    $uploadOk = 0;
  }

  // Verification de la taille du fichier
  if ($_FILES["inputGroupFile01"]["size"] > 500000) {
    header("Location: ../reserver.php?error=Désolé, le fichier est trop lourd.");
    $uploadOk = 0;
  }

  // Verification de l'extension du fichier
  if($imageFileType != "pdf" && $imageFileType != "jpg") {
    header("Location: ../reserver.php?error=Désolé, seuls les fichiers PDF et JPG sont autorisés.");
    $uploadOk = 0;
  }
  ...
  // si tout est bon, on essaie d'upload le fichiers
    ...
    if (move_uploaded_file($_FILES["inputGroupFile01"]["tmp_name"], $target_file)) {
      header("Location: ../reserver.php?success=Le fichier a été importé avec succès.");
    } else {
      header("Location: ../reserver.php?error=Désolé, il y a eu une erreur lors de l'importation de votre fichier.");
    }
  }
}
?>